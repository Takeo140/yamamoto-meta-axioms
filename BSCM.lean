...
/-- 
  【BSCMの遷移関数 δ】
  状態空間 S を 16ビット（0 ≤ s ≤ 65535）の有限集合として定義。
...
def bscm_delta (s : Nat) : Nat :=
  if s % 2 = 0 then
    ...
    65535 - (s % 65536)    -- 状態攪乱（16ビット空間での完全ビット反転）
...
/-- 
  【BSCM空間不変性定理（有界性の証明）】
  初期状態が 16ビットのレジスタ幅（65535）に収まっている限り、
...
theorem bscm_state_bounded (s : Nat) (h : s ≤ 65535) : bscm_delta s ≤ 65535 := by
  ...
  · -- ミキシング相の有界性
    omega
  · -- 反転（XOR）相の有界性
    -- 65535 から任意の剰余を引いた値は必ず 65535 以下になる性質を omega が自動認識
    omega

/-- 任意の時間軸（k）において、計算機が永久に有限空間に閉じ込められていることの帰納的証明 -/
theorem bscm_machine_never_overflows (initial_state : Nat) (h_init : initial_state ≤ 65535) (k : Nat) : 
